<?php
/**
 * Questa interfaccia rappresenta la classe ( e subclassi )  OptionsPage
 *
...
class Net7_AdminOptionsPage
{

    /**
     * @var string - defaul tab if isn't set in settings arguments
     */
    public $default_tab = 'general_options';

    /**
     * @var string - label for default tab
     */
    public $default_tab_label = 'Options';
    ...
    function __construct( $plugin_name, $version, $plugin_prefix )
    {
            ...
            )
    {
        ...
        $this->page_title = $page_title != "" ? $page_title : $this->plugin_name;
        $this->menu_title = $menu_title != "" ? $menu_title : $this->plugin_name;
        $this->menu_slug = $menu_slug != "" ? sanitize_title($menu_slug) : sanitize_title($this->plugin_name);
    ...
    public function render_menu_page()
    {
      //  $this->user_can_see_this_page();
        ...
        <p><?php echo $this->page_title ?> is a plugin developed by <a href="http://netseven.it" target="_blank">NET7 srl</a></p>
        <div class="wrap">
            <?php
    ...
    *  Retrieve settings_group of current tab
    ...
    protected function user_can_see_this_page()
    {
        ...
        {
            wp_die(_x('You do not have sufficient permissions to access this page.' , 'WebMapp option page capability error' ) );
        }
    }
        ...
        foreach ( $this->settings as $setting_name => $setting_args ) {
            ...
            if ( $setting_args['tab'] == $this->current_tab ) {
                ...
                if ( get_option( $setting_name ) ) {
                    ...
                    $value = isset( $setting_args['value'] ) ? $setting_args['value'] : "" ;
                }
                ...
                } elseif ( isset( $setting_args['html_tab'] ) ) {
                    ...
                    switch ( $type )
                    {
                        ...
                        case 'multi_checkbox':
                            if ( isset( $setting_args['options'] ) && is_array($setting_args['options'] ) )
                            {
                                $options = $setting_args['options'];
                                foreach ( $options as $option_value => $option_label ) :
                                    $selected = $value !== "" && in_array( $option_value, $value ) ? "checked=\"checked\"" : "";
                                    echo "<input type=\"checkbox\" name=\"" . $setting_name . "[]\"  value=\"$option_value\" $selected/> $option_label<br>";
                                endforeach;
                            }
                            break;
                        case 'radio':
                            if ( isset( $setting_args['options'] ) && is_array($setting_args['options'] ) )
                            {
                                ...
                                foreach ( $options as $option_value => $option_label ) :
                                    $selected = $value !== "" && in_array( $option_value, $value ) ? "checked=\"checked\"" : "";
                                    echo "<input type=\"radio\" name=\"" . $setting_name . "[]\"  value=\"$option_value\" $selected/> $option_label<br>";
                                endforeach;
                            }
                            break;
                        case 'checkbox':
                        ...
                        case 'media':
                            echo Utils::upload_files_input( $value, $setting_name );
                            break;
                        ...
                        case 'select_page':
                            wp_dropdown_pages(array(
                                'selected' => $value,
                                'name' => $setting_name,
                                'show_option_none' =>  __("no page selected", MURUCA_CORE_TEXTDOMAIN)
                            ));
                            if($value != "") {
                                echo "<a href='" . get_edit_post_link($value) . "'>" . __("edit page", MURUCA_CORE_TEXTDOMAIN) . "</a> " ;
                                echo "<a href='" . get_permalink($value) . "'>" . __("view page", MURUCA_CORE_TEXTDOMAIN) . "</a>" ;
                            }
                            break;
